	...
	var   log 			= require( "ee-log" )
		...
		, sign 			= require( "../../ee-aws-v4-signature" ) // ( "ee-aws-v4-signature" )
		, req 			= require( "request" )
		...
		, request: function( options, callback ){				
			...
			req( {
				...
				if ( err ) callback( err );
					...
					var data = this.services.decode( body, function( err, data ){
						if ( err ) callback( err );
						else {
							if ( res.statusCode !== 200 ){
								callback( this.services.getErrorDetail( res.statusCode, data ), res.statusCode, data );
							}
